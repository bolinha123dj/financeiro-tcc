/* ============================================================
   api.js — Toda comunicação com o backend (CORRIGIDO)
   ============================================================ */

const API = (() => {

  // URL corrigida para o ambiente de produção no Render
  const API-URL  = 'https://financeiro-tcc-backend.onrender.com/api';
  ...
  async function request(path, opts = {}, auth = true) {
    ...
    try {
      ...
      // Erro de rede geralmente significa que o servidor no Render está "acordando" ou a URL está inacessível
      throw new Error('Não foi possível conectar ao servidor. Se for o primeiro acesso, aguarde 30 segundos para o servidor "acordar" e tente novamente.');
    }
